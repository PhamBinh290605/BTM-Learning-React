...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.code === 1000) {
        toast.success("Đăng ký thành công! Chào mừng " + requestData.email);
        navigate("/auth/login");
      } else {
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="relative">
          <input
            ...
            required
            className="w-full bg-white/5 border border-white/12 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
          />
          <button
            type="button"
